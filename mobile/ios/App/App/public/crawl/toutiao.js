...
function groupIdFromContent(url, html) {
  const fromUrl = groupIdFromUrl(url);
...
/** 短链跳转后解析作品 ID（iOS Capacitor 代理下 resp.url 可能不是最终地址，需从 HTML 兜底） */
async function resolveSharePage(shareUrl) {
  const res = await httpGet(shareUrl.trim(), { headers: mobileHeaders() });
  const html = String(res.data || "");
  const groupId =
    groupIdFromContent(res.url, html) || groupIdFromContent(shareUrl, html);
  if (!groupId) throw new Error("无法解析头条作品 ID");
  const finalUrl =
    groupIdFromUrl(res.url) && !String(res.url).includes("_capacitor_http_interceptor")
      ? res.url
      : `https://m.toutiao.com/video/${groupId}/`;
  return { finalUrl, groupId };
}
...
async function resolvePlayUrls(article, referer) {
  ...
  const playUrls = pickPlayUrls(parseJsonPayload(res.data));
...
export async function tryCrawlToutiao(shareText, onProgress) {
  ...
  try {
    const { finalUrl, groupId } = await resolveSharePage(shareUrl);
    ...
    throw new Error(err?.message || "头条爬取失败");
  }
}
